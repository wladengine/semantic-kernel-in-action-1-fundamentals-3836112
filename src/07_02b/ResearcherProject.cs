...
public class ResearcherProject
{
	...
	private async Task<string> ProcessAndSummarizeSearchResults(
		...
		int numOfResultsToSummarize = 5) // BE CAREFUL TO INCREASE THIS VALUE!!! THIS IS EXACT AMOUNT OF KERNEL CALLS WITH ENORMOUS AMOUNT OF TOKENS EACH TIME!!!
	{
		...
		var customHandlebarsPromptResult = await kernel.InvokeAsync(
			...
			new() 
			{
				{ "webSearchResults", webSearchResults.Take(numOfResultsToSummarize) },
